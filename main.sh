...
# // Repository
LT_REP0="https://raw.gitubusercontent.com/LunaticVpns/Parakan/main/"
...
TIMKU="10"
IDDKU="6256307021"
KEYKU="7077823927:AAGwDpiQqPI6O8cXyNvhhrDT5kDRSvePx90"
LINKU="https://api.telegram.org/bot$KEY/sendMessage"
...
clear
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/puncakstar3/pro1/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/puncakstar3/pro1/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
Exp1=$(curl https://raw.githubusercontent.com/puncakstar3/pro1/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
echo -e "\e[32mloading...\e[0m"
clear
REPO="https://raw.githubusercontent.com/puncakstar3/pro1/main/"
NOOBZJSON="https://raw.githubusercontent.com/LunaticBackend/noobzvpns/main/"
start=$(date +%s)
secs_to_human() {
...
wget https://raw.githubusercontent.com/puncakstar3/pro1/main/Fls/cf.sh && chmod +x cf.sh && ./cf.sh
rm -f /root/cf.sh
clear
else
...
clear
restart_system() {
USRSC=$(wget -qO- https://raw.githubusercontent.com/puncakstar3/pro1/main/ip | grep $ipsaya | awk '{print $2}')
EXPSC=$(wget -qO- https://raw.githubusercontent.com/puncakstar3/pro1/main/ip | grep $ipsaya | awk '{print $3}')
...
# // Delete Data .db
rm -rf /etc/LT/vmess/.vmess.db
rm -rf /etc/LT/vless/.vless.db
rm -rf /etc/LT/trojan/.trojan.db
rm -rf /etc/LT/shadowsocks/.shadowsocks.db
rm -rf /etc/LT/ssh/.ssh.db
rm -rf /etc/LT/bot/.bot.db
rm -rf /etc/LT/noobz/.noobzvpns.db

# // Repo Protocol
mkdir -p /etc/LT/noobzvpns
mkdir -p /etc/LT/bot
mkdir -p /etc/LT/xray
mkdir -p /etc/LT/vmess
mkdir -p /etc/LT/vless
mkdir -p /etc/LT/trojan
mkdir -p /etc/LT/shadowsocks
mkdir -p /etc/LT/ssh
...
# // limit ip
mkdir -p /etc/LT/limit/vmess/ip
mkdir -p /etc/LT/limit/vless/ip
mkdir -p /etc/LT/limit/trojan/ip
mkdir -p /etc/LT/limit/ssh/ip
mkdir -p /etc/LT/limit/shadowsocks/ip
mkdir -p /etc/LT/limit/noobzvpns/ip

# // limit quota
mkdir -p /etc/LT/limit/vmess/quota
mkdir -p /etc/LT/limit/vless/quota
mkdir -p /etc/LT/limit/trojan/quota
mkdir -p /etc/LT/limit/ssh/quota
mkdir -p /etc/LT/limit/shadowsocks/quota
mkdir -p /etc/LT/limit/noobzvpns/quota

# // Repo Menu
mkdir -p /etc/Cdy/LT
# // Files Protocol
mkdir -p /etc//LTfiles/vmess
mkdir -p /etc/LT/files/vless
mkdir -p /etc/LT/files/trojan
mkdir -p /etc/LT/files/ssh
mkdir -p /etc/LT/files/shadowsocks
mkdir -p /etc/LT/files/noobzvpns
...
# // Data Db Account
touch /etc/LT/vmess/.vmess.db
touch /etc/LT/vless/.vless.db
touch /etc/LT/trojan/.trojan.db
touch /etc/LT/shadowsocks/.shadowsocks.db
touch /etc/LT/ssh/.ssh.db
touch /etc/LT/bot/.bot.db
touch /etc/LT/.noobzvpns.db

# // plughin Data .db
echo "& plughin Account" >>/etc/LT/vmess/.vmess.db
echo "& plughin Account" >>/etc/LT/vless/.vless.db
echo "& plughin Account" >>/etc/LT/trojan/.trojan.db
echo "& plughin Account" >>/etc/LT/shadowsocks/.shadowsocks.db
echo "& plughin Account" >>/etc/LT/ssh/.ssh.db
echo "& plughin Account" >>/etc/LT/noobzvpns/.noobzvpns.db
...
wget -O /etc/LT/config.json "${NOOBZJSON}config.json" >/dev/null 2>&1
...
wget "http://raw.githubusercontent.com/puncakstar3/pro1/main/Fls/limit.sh && chmod +x limit.sh && ./limit.sh"
cd
wget -q -O /etc/xray/LT/limit-ip "${REPO}Fls/limit-ip"
chmod +x /etc/xray/LT/*
cd /usr/bin
sed -i 's/\r//' limit-ip
cd
...
ExecStart=/etc/xray/LT/limit-ip vmip
Restart=always
[Install]
WantedBy=multi-user.target
EOF
...
[Service]
WorkingDirectory=/root
ExecStart=/etc/xray/LT/limit-ip shadowsocks
Restart=always
[Install]
WantedBy=multi-user.target
EOF
...
[Service]
WorkingDirectory=/root
ExecStart=/etc/xray/LT/limit-ip vlip
Restart=always
[Install]
WantedBy=multi-user.target
EOF
...
[Service]
WorkingDirectory=/root
ExecStart=/etc/xray/LT/limit-ip trip
Restart=always
[Install]
WantedBy=multi-user.target
EOF
...
mkdir -p /usr/local/LT/
wget -q -O /usr/local/LT/udp-mini "${REPO}Fls/udp-mini"
chmod +x /usr/local/LT/udp-mini
